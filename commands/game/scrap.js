...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let userlab = await getDevotions(interaction.user.id);
      ...
      if (userlab[i] > 0) {
        ...
        container.addTextDisplayComponents(
          new TextDisplayBuilder().setContent(
            `${classes[i].emoji} **${classes[i].name}:** ${userlab[i] % 40}/40 | Reward: ${rewardemoji}\n${progressbar}`
          )
        );
        labcovered += 1;
    ...
    if (labcovered == 0) {
      container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`☮️ You haven't devoted any emojis yet.`));
    }

    let help = new ButtonBuilder().setCustomId("help").setLabel("Help").setEmoji("❔").setStyle(ButtonStyle.Primary);

    container.addActionRowComponents(new ActionRowBuilder().addComponents(help));
