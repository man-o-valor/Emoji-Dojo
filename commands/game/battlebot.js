...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("battlebot")
    .setDescription("Battle DojoBot, with no chance of losing coins (You can reject Dojobot every 10 minutes)")
  ...
  async execute(interaction) {
    if (await setupUser(interaction.user)) {
      ...
      if (bp < Date.now() / 1000 && bbcd < Date.now() / 1000 && bp !== true) {
        ...
        let rerolltime = (await database.get(interaction.user.id + "botreroll")) ?? "0";
        ...
        if (rerolltime > Date.now() / 1000) {
          ...
          nah.setLabel("Reroll (wait " + Math.ceil((rerolltime - Date.now() / 1000) / 60) + " min)");
        }
        ...
        let dojobotsquad = (await database.get(interaction.user.id + "dojobotsquad")) ?? "";
        if (dojobotsquad == "") {
          ...
          await database.set(interaction.user.id + "dojobotsquad", player2squadarray);
        } else {
        ...
        if (player2squadarray == "error") {
          ...
          for (let i = 0; i < 8; i++) {
            gamedata.squads[1].push(new BattleEmoji(player2squadarray[i], 2, "DojoBot"));
          }
          ...
          let challengecontainer = new ContainerBuilder()
            .addTextDisplayComponents(
              new TextDisplayBuilder().setContent(
                `### \`@DojoBot\`, <@${interaction.user.id}> wants to battle with you!${
                  cursed == 1 ? " 👺" : ""
                } • Speed: ${battlespeed}`
              )
            )
          ...
          try {
            collector.on("collect", async (interaction2) => {
              let bp2 = await database.get(interaction.user.id + "battlepending");
              if (bp2 !== true) {
                if (interaction2.customId === "battle") {
                  await database.set(interaction.user.id + "battlepending", true, 300000);
                  ...
                  if (gamedata.turn >= 200 || (gamedata.squads[0].length == 0 && gamedata.squads[1].length == 0) || gamedata.draw) {
                    ...
                    if (gamedata.squads[1].length == 0) {
                      await restockCoins(interaction.user.id);
                      let mult = parseInt(await database.get(interaction.user.id + "coinmod"));
                      let diff1 = curveCoins(gamedata.squads[0].length, mult);
                      coinsdata = await changeCoins(interaction.user.id, diff1, true);
                      diff1 = coinsdata[0];
                    ...
                    if (gamedata.squads[0].length == 0) {
                      await changeCoins(interaction.user.id, 0);
                      battleendcontainer = new ContainerBuilder()
                        .addTextDisplayComponents(
                          new TextDisplayBuilder().setContent(
                      ...
                      logs.logs.players[`user${interaction.user.id}`].botlosses =
                        logs.logs.players[`user${interaction.user.id}`].botlosses ?? 0;
                  ...
                  collector.on("collect", async (interaction3) => {
                    try {
                      ...
                      logs.logs.players[`user${interaction3.user.id}`].botlogsrequested =
                        logs.logs.players[`user${interaction3.user.id}`].botlogsrequested ?? 0;
                  ...
                  await database.set(interaction.user.id + "dojobotsquad", player2squadarray);
                  await database.set(interaction.user.id + "botreroll", 20 * 60 + Math.floor(Date.now() / 1000));
                  player1squadtext = "";
                  for (let i = 7; i > -1; i--) {
                    player1squadtext += `${emojis[player1squadarray[i]].emoji} `;
                  }
                  ...
                  for (let i = 0; i < 8; i++) {
                    gamedata.squads[0].push(new BattleEmoji(player1squadarray[i], 1));
                  }
                  for (let i = 0; i < 8; i++) {
                    gamedata.squads[1].push(new BattleEmoji(player2squadarray[i], 2));
                  }
