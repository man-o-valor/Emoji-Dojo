              ...
              collector.on("collect", async (interaction2) => {
                ...
                if (interaction2.customId === "battle" && accepts[0] == 1 && accepts[1] == 1) {
                  ...
                  logs.logs.players[`user${interaction.user.id}`].userstarted =
                    logs.logs.players[`user${interaction.user.id}`].userstarted ?? 0;
                  ...
                  if (gamedata.turn >= 200 || (gamedata.squads[0].length == 0 && gamedata.squads[1].length == 0) || gamedata.draw) {
                    ...
                    logs.logs.players[`user${interaction.user.id}`].userdraws =
                      logs.logs.players[`user${interaction.user.id}`].userdraws ?? 0;
                    ...
                    if (gamedata.squads[1].length == 0) {
                      diff1 = 40;
                      diff2 = -40;
                      coinsdata = await changeCoins(interaction.user.id, diff1);
                      diff1 = coinsdata[0];
                      doublerbonus = coinsdata[1];
                      let bonusmsg = doublerbonus > 0 ? ` (💫 ${doublerbonus})` : "";
                      await changeCoins(battleuser.id, diff2);
                      ...
                      logs.logs.players[`user${interaction.user.id}`].userwins =
                        logs.logs.players[`user${interaction.user.id}`].userwins ?? 0;
                      logs.logs.players[`user${interaction.user.id}`].userwins += 1;
                      logs.logs.players[`user${battleuser.id}`].userlosses =
                        logs.logs.players[`user${battleuser.id}`].userlosses ?? 0;
                      ...
                      for (let i = 7; i > -1; i--) {
                        logs.logs.emojis[player2squadarray[i]].userlosses =
                          logs.logs.emojis[player2squadarray[i]].userlosses ?? 0;
                    ...
                    if (gamedata.squads[0].length == 0) {
                      ...
                      diff1 += doublerbonus;
                      let bonusmsg = doublerbonus > 0 ? ` (💫 ${doublerbonus})` : "";
                      await changeCoins(battleuser.id, diff1);
                      diff2 = -40;
                      await changeCoins(interaction.user.id, diff2);
                      ...
                      battleendcontainer = new ContainerBuilder()
                        ...
                        .addActionRowComponents(row2);
                      let logs = await getLogs();
                      logs.logs.players[`user${interaction.user.id}`].userlosses =
                        logs.logs.players[`user${interaction.user.id}`].userlosses ?? 0;
                      ...
                      for (let i = 7; i > -1; i--) {
                        logs.logs.emojis[player1squadarray[i]].userlosses =
                          logs.logs.emojis[player1squadarray[i]].userlosses ?? 0;
                  ...
                  collector.on("collect", async (interaction3) => {
                    try {
                      ...
                      logs.logs.players[`user${interaction3.user.id}`].userlogsrequested =
                        logs.logs.players[`user${interaction3.user.id}`].userlogsrequested ?? 0;
