        ...
        if (gamedata.squads[squad-1][pos].hp <= 0) {
            ...
            if (val>0) { 
                ...
                if ((gamedata.squads[squad-1][pos] ?? {id:undefined}).id == 67) { // lock with ink pen
                    const tempemj = gamedata.squads[squad-1][0].emoji
                    const temphp = gamedata.squads[squad-1][0].hp
                    const tempdmg = gamedata.squads[squad-1][0].dmg + 1
                    gamedata.squads[squad-1].splice(0,1,lodash.cloneDeep(emojis[68]))
                    gamedata.squads[squad-1][0].hp = temphp
                    gamedata.squads[squad-1][0].dmg = tempdmg
                    gamedata = richtextadd(gamedata,`\n🔒 ${gamedata.player[squad2-1]}'s ${emojis[67].emoji} transformed ${gamedata.player[squad-1]}'s ${tempemj} into a ${emojis[68].emoji}, and increased its attack power by 1!`)
                }
